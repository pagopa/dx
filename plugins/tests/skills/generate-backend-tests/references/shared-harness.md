...
- prefer externally observable artifacts over in-process hooks or private runtime state when both can prove the same contract
- use private fields, unsupported casts, or internal instrumentation only as last-resort diagnostics, not as the primary oracle; document why no external oracle exists when they are necessary
