...
| `TF-G04` | A resource needs a name or a new subnet | Ensure the relevant `pagopa-dx/azure` or `pagopa-dx/aws` provider is configured. Use `provider::dx::resource_name()` for every supported resource name. Build or reuse its naming configuration from `prefix`, `env_short`, `location`, `domain`, `app_name`, and `instance_number`. Use `dx_available_subnet_cidr` for every new subnet; never calculate or hardcode a new subnet CIDR. | Required only when a base naming value cannot be inferred. |
| `TF-G05` | A DX Registry module is added or upgraded | Derive its current version from the module `package.json` and pin it with `~> major.minor`. | Required only for a major-version migration or intentional older version. |
| `TF-G06` | Terraform is placed in an environment root or local module | Keep environment configuration in root `locals.tf`; never add root `variables.tf`. Group resources by logical service in `_modules/` when the service owns multiple related resources or is reused. | Required only when more than one structure is equally consistent with the repository. |
| `TF-G07` | Resources or modules accept tags | Reuse the target environment's required tags and apply them to every taggable resource and module. Do not invent ownership or cost-allocation values. | Required when required tag values cannot be inferred. |
| `TF-G08` | The change introduces a new service or technology choice | Check the Technology Radar. Prefer `adopt`; explain `trial`; require confirmation for `assess` and `hold`. Add a `# radar: hold` comment for confirmed `hold` usage. | Required for `assess`, `hold`, or an unavailable Radar result. |
| `TF-G09` | Terraform is generated or modified | Produce complete code without placeholder comments or deferred required wiring. | No. |
| `TF-G10` | The implementation is complete | Run `terraform init` or `terraform init -backend=false`, `terraform validate`, applicable `terraform plan`, and the smallest repository validation covering the change. Investigate and fix actionable errors; never report the implementation as complete while an applicable check fails. | No; report unavailable credentials or backend access. |
