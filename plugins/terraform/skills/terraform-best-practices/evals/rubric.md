...
| Evidence-backed inference | `TF-G07` | Repository values may already exist | Factual values are inferred from named files or module instances rather than guessed. |
...
| Naming, environment contract, and subnet allocation | `TF-G04` | Names or new subnets are introduced | The DX provider is configured, base environment fields are mapped to `provider::dx::resource_name()`, and new subnets use `dx_available_subnet_cidr`. |
| Version pinning | `TF-G05` | A DX Registry module is added or upgraded | `~> major.minor` is derived from the module `package.json`. |
| Structure correctness | `TF-G06` | New resources or composition are introduced | Root configuration uses locals and service-owned resources are grouped appropriately. |
| Required tags | `TF-G07` | Taggable resources or modules are introduced | Existing required tags are inferred and applied consistently to every taggable resource and module. |
| Technology Radar alignment | `TF-G08` | A new service or technology is selected | Non-recommended or unavailable Radar results require confirmation. |
| Complete implementation | `TF-G09` | Terraform is generated | Required code and supporting resources contain no placeholders. |
| Validation | `TF-G10` | Terraform is modified | The smallest applicable init, validate, plan, and repository checks run or are reported unavailable. |
...
- **DX conventions:** applicable `TF-G01`, `TF-G04`, `TF-G05`, `TF-G06`, and `TF-G07` criteria average at least `1.75`.
