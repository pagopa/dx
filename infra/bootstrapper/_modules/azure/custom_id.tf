...
resource "azurerm_federated_identity_credential" "certificates_cd" {
  name      = "dx-environment-infra-${local.env_long}-certificates-${local.env_long}-cd"
  audience  = ["api://AzureADTokenExchange"]
  issuer    = "https://token.actions.githubusercontent.com"
  parent_id = module.bootstrap.identities.infra.cd.id
  subject   = "repo:pagopa/${var.repository.name}:environment:certificates-${local.env_long}-cd"
}
