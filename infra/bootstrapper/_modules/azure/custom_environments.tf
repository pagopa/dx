...
resource "github_actions_environment_secret" "automation_cd_client_id" {
  repository      = var.repository.name
  environment     = "automation-${local.env_long}-cd"
  secret_name     = "ARM_CLIENT_ID"
  plaintext_value = module.bootstrap.identities.infra.cd.client_id
}

resource "github_actions_environment_secret" "automation_cd_subscription_id" {
  repository      = var.repository.name
  environment     = "automation-${local.env_long}-cd"
  secret_name     = "ARM_SUBSCRIPTION_ID"
  plaintext_value = module.bootstrap.subscription_id
