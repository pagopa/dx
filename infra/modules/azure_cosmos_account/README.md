...
|------|-------------|------|---------|:--------:|
| <a name="input_alerts"></a> [alerts](#input\_alerts) | (Optional) Alerts configuration for Cosmos DB account. | <pre>object({<br>    enabled         = optional(bool, true)<br>    action_group_id = optional(string, null)<br>    thresholds = optional(object({<br>      provisioned_throughput_exceeded = optional(number, null)<br>    }), {})<br>  })</pre> | <pre>{<br>  "enabled": true<br>}</pre> | no |
| <a name="input_automatic_failover_enabled"></a> [automatic\_failover\_enabled](#input\_automatic\_failover\_enabled) | (Optional) Whether Automatic Failover is enabled for Cosmos DB account or not. | `bool` | `false` | no |
| <a name="input_consistency_policy"></a> [consistency\_policy](#input\_consistency\_policy) | Defines the consistency policy for CosmosDB. Defaults to 'Session' if not specified. | <pre>object({<br>    consistency_level       = string<br>    max_interval_in_seconds = optional(number)<br>    max_staleness_prefix    = optional(number)<br>  })</pre> | n/a | yes |
