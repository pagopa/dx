...
resource "github_repository_environment_deployment_policy" "infra_cd_branch" {
  for_each = { for k, v in setproduct(var.repository.environments, local.infra_cd_policy_branches) : "${v[0]}-${v[1]}" => { env = v[0], branch = v[1] } }
...
resource "github_repository_environment_deployment_policy" "app_cd_branch" {
  for_each = { for k, v in setproduct(var.repository.environments, local.app_cd_policy_branches) : "${v[0]}-${v[1]}" => { env = v[0], branch = v[1] } }
...
resource "github_repository_environment_deployment_policy" "opex_cd_branch" {
  for_each = { for k, v in setproduct(var.repository.environments, local.opex_cd_policy_branches) : "${v[0]}-${v[1]}" => { env = v[0], branch = v[1] } }
...
resource "github_repository_environment_deployment_policy" "bootstrapper_cd_branch" {
  for_each = { for k, v in setproduct(var.repository.environments, local.bootstrapper_cd_policy_branches) : "${v[0]}-${v[1]}" => { env = v[0], branch = v[1] } }
