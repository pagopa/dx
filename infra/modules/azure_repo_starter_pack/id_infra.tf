...
resource "azurerm_role_assignment" "infra_ci_rg_rbac_kv_secr" {
  scope                = azurerm_resource_group.main.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = azurerm_user_assigned_identity.infra_ci.principal_id
}

resource "azurerm_role_assignment" "infra_ci_rg_rbac_kv_cert" {
  scope                = azurerm_resource_group.main.id
...
resource "azurerm_role_assignment" "infra_cd_rg_rbac_kv_secr" {
  scope                = azurerm_resource_group.main.id
  role_definition_name = "Key Vault Secrets Officer"
  principal_id         = azurerm_user_assigned_identity.infra_cd.principal_id
}

resource "azurerm_role_assignment" "infra_cd_rg_rbac_kv_cert" {
  scope                = azurerm_resource_group.main.id
