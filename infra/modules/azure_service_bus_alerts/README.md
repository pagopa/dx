...
| <a name="input_resource_group_name"></a> [resource\_group\_name](#input\_resource\_group\_name) | Optional. Name of the resource group where the alert will be created. When null (the default), locals.tf uses coalesce(...) to derive the resource group name from service\_bus\_namespace\_id; if a non-null value is provided, it overrides the derived value. | `string` | `null` | no |
