resource "azurerm_monitor_metric_alert" "active" {
  name                = provider::dx::resource_name(merge(local.naming_config, { resource_type = "monitor_alert_sbns_active" })) # requires a provider update. Expected value: dx-d-itn-domain-sbns-active-ma-01
