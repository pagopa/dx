...
# Resource Group
resource "azurerm_role_assignment" "admin_group_rgs" {
...
# Key Vault
resource "azurerm_role_assignment" "admin_group_rgs_kv_data" {
  for_each = local.resource_group_ids
...
resource "azurerm_role_assignment" "admin_group_rgs_kv_admin" {
  for_each = local.resource_group_ids
