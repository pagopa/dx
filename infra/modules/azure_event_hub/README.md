...
| <a name="input_allowed_sources"></a> [allowed\_sources](#input\_allowed\_sources) | Lists of allowed sources (subnets and ip masks) | <pre>object({<br>    subnet_ids = list(string)<br>    ips        = list(string)<br>  })</pre> | <pre>{<br>  "ips": [],<br>  "subnet_ids": []<br>}</pre> | no |
