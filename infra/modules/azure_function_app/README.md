...
| <a name="input_entra_id_authentication"></a> [entra\_id\_authentication](#input\_entra\_id\_authentication) | Enables Entra ID authentication on the Function App, allowing callers (e.g. APIM) to authenticate via their Managed Identity instead of using function keys. When set, configures auth\_settings\_v2 with an Active Directory v2 identity provider. When null (default), no authentication layer is added and the Function App uses classic key-based auth. | <pre>object({<br/>    entra_application_client_id = string<br/>    allowed_client_applications = list(string)<br/>    tenant_id                   = string<br/>  })</pre> | `null` | no |
...
| <a name="output_entra_id_authentication"></a> [entra\_id\_authentication](#output\_entra\_id\_authentication) | Indicates whether Entra ID authentication (via caller Managed Identity) is enabled and the Entra application client ID used. Useful for downstream APIM policy configuration. |
