...
| <a name="input_customer_managed_key"></a> [customer\_managed\_key](#input\_customer\_managed\_key) | (Optional) Customer managed key to use for encryption. Currently type can only be set to 'kv'. If the key vault is in the same account, and key\_name is not set, the key and relevant permissions will be automatically created. | <pre>object({<br>    enabled                   = optional(bool, false)<br>    type                      = optional(string, null)<br>    key_name                  = optional(string, null)<br>    user_assigned_identity_id = optional(string, null)<br>    key_vault_id              = optional(string, null)<br>  })</pre> | <pre>{<br>  "enabled": false<br>}</pre> | no |
