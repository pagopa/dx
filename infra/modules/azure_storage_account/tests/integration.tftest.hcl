...
run "apply_development" {
  ...
  variables {
    environment                         = merge(var.environment, { app_name = "sa-dev", instance_number = run.setup.instance_numbers.development })
    tags                                = var.tags
    resource_group_name                 = run.setup.resource_group_name
...
run "apply_delegated_access" {
  ...
  variables {
    environment         = merge(var.environment, { app_name = "sa-da", instance_number = run.setup.instance_numbers.delegated_access })
    tags                = var.tags
    resource_group_name = run.setup.resource_group_name
...
run "apply_public_network" {
  ...
  variables {
    environment                         = merge(var.environment, { app_name = "sa-pub", instance_number = run.setup.instance_numbers.public_network })
    tags                                = var.tags
    resource_group_name                 = run.setup.resource_group_name
    use_case                            = "default"
    force_public_network_access_enabled = true
