...
| <a name="input_enable_waf"></a> [enable\_waf](#input\_enable\_waf) | Whether to enable WAF for enhanced protection. Enable this for sensitive production environments. Be aware that enabling WAF will incur additional costs. | `bool` | `false` | no |
...
| <a name="input_node_major_version"></a> [node\_major\_version](#input\_node\_major\_version) | The major version of the runtime to use for the lambda function. Allowed values are 20 or 22. | `string` | `"22"` | no |
| <a name="input_server"></a> [server](#input\_server) | The server lambda function configuration. | <pre>object({<br/>    environment_variables = optional(map(string), {})<br/>    lambda_layers         = optional(list(string), [])<br/>  })</pre> | <pre>{<br/>  "environment_variables": {},<br/>  "lambda_layers": []<br/>}</pre> | no |
