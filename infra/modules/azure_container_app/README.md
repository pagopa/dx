...
| <a name="input_restrict_access_from_within_environment"></a> [restrict\_access\_from\_within\_environment](#input\_restrict\_access\_from\_within\_environment) | If \"true\", the container app is accessible only from other apps on the same Container App Environment.<br/>If \"true\" (default), the access is managed by the Container App Environment." | `bool` | `false` | no |
