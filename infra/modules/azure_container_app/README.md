...
| <a name="input_restrict_access_from_within_environment"></a> [restrict\_access\_from\_within\_environment](#input\_restrict\_access\_from\_within\_environment) | If true (default), the Container App is accessible via public internet or within the VNet, depending on the Container App Environment configuration.<br/>If false, the container app is accessible only from other apps on the same Container App Environment. | `bool` | `false` | no |
| <a name="input_secrets"></a> [secrets](#input\_secrets) | List of Key Vault secret references to define in the Container App.<br/>Secrets are exposed to containers only when explicitly referenced in `container_app_templates[*].secret_names`.<br/>To remove a secret without downtime, first deploy the application version that no longer needs it, then remove it from every container `secret_names` list, and finally remove it from `secrets`. | <pre>list(object({<br/>    name                = string<br/>    key_vault_secret_id = string<br/>  }))</pre> | `[]` | no |
