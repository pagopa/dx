...
| <a name="input_secrets"></a> [secrets](#input\_secrets) | List of Key Vault secret references to define in the Container App.<br/>Set `use_in_container` to `false` to keep a secret available at Container App Environment level without exposing it as a container environment variable (useful for secrets deletion). | <pre>list(object({<br/>    name                = string<br/>    key_vault_secret_id = string<br/>    use_in_container    = optional(bool, true)<br/>  }))</pre> | `[]` | no |
