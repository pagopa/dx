...
- **Flexible Ingress**: Configure external (public) or internal-only access via the `public_access_enabled` variable. Public access is enabled by default, but only becomes effective when the parent Container App Environment has `internal_load_balancer_enabled` set to false.
