...
variable "environment" {
  ...
  validation {
    condition     = length("${var.environment.prefix}${var.environment.env_short}reg${var.environment.domain == null ? "" : replace(var.environment.domain, "-", "")}${var.environment.app_name}-mysql-replica-${var.environment.instance_number}") <= 63
    error_message = "Azure MySQL Flexible Server name must contain between 3 and 63 characters. Current value is \"${var.environment.prefix}${var.environment.env_short}reg${var.environment.domain == null ? "" : var.environment.domain}${var.environment.app_name}-mysql-replica-${var.environment.instance_number}\""
  }
...
variable "db_version" {
  type        = string
  description = "The version of MySQL Flexible Server to use. Possible values are 5.7, and 8.0.21"
  default     = "8.0.21"
}
...
variable "administrator_credentials" {
  ...
  sensitive   = true
  description = "Flexible MySQL server administrator credentials (Only for tests)"
}
...
variable "backup_retention_days" {
  type        = number
  description = "(Optional) The backup retention days for the MySQL Flexible Server. Possible values are between 7 and 35 days."
  default     = 7
}

variable "zone" {
  type        = number
  description = "(Optional) Specifies the Availability Zone in which the MySQL Flexible Server should be located."
  default     = null
}

variable "replica_zone" {
  type        = number
  description = "(Optional) Specifies the Availability Zone in which the Replica MySQL Flexible Server should be located."
  default     = null
}
