...
| <a name="input_service_bus"></a> [service\_bus](#input\_service\_bus) | A list of Service Bus role assignments | <pre>list(object({<br/>    namespace_name      = string<br/>    resource_group_name = string<br/>    queue_names         = optional(list(string), [])<br/>    topic_names         = optional(list(string), [])<br/>    subscriptions       = optional(map(string), {}) # Terraform processes map literals with duplicate keys by taking the last defined instance.<br/>    role                = string<br/>    description         = string<br/>  }))</pre> | `[]` | no |
