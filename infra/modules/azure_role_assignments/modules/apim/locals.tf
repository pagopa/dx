locals {
  norm_apims = [for apim in var.apim : {
    name                = apim.name
    id                  = provider::azurerm::normalise_resource_id("/subscriptions/${var.subscription_id}/resourceGroups/${apim.resource_group_name}/providers/Microsoft.ApiManagement/service/${apim.name}")
