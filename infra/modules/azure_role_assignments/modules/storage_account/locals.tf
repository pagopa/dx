locals {
  norm_blobs = [
    for blob in var.storage_blob : {
      ...
      storage_account_id            = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${blob.resource_group_name}/providers/Microsoft.Storage/storageAccounts/${blob.storage_account_name}"
