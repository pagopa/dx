locals {
  ...
  norm_caches = [for cache in var.redis : {
    ...
    cache_id            = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${cache.resource_group_name}/providers/Microsoft.Cache/Redis/${cache.cache_name}"
