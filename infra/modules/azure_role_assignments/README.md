...
| <a name="input_managed_redis"></a> [managed\_redis](#input\_managed\_redis) | List of Azure Managed Redis (AMR) instance resource IDs to grant data-plane access to.<br/><br/>Each entry is a full Azure resource ID<br/>(e.g., /subscriptions/{subId}/resourceGroups/{rgName}/providers/Microsoft.Cache/redisEnterprise/{name}).<br/><br/>AMR assigns the built-in "default" access policy (full data-plane access).<br/>There are no role choices; access is binary (granted or not).<br/><br/>Note: this variable targets Azure Managed Redis. For legacy Azure Cache for<br/>Redis, use the `redis` variable instead. | `list(string)` | `[]` | no |
