...
func TestResourceNameFunction_DomainWithHyphen(t *testing.T) {
	t.Parallel()
	// Test that a domain containing a hyphen causes an error
	resource.UnitTest(t, resource.TestCase{
		TerraformVersionChecks: []tfversion.TerraformVersionCheck{
			tfversion.SkipBelow(tfversion.Version1_8_0),
		},
        ...
        `,
				ExpectError: regexp.MustCompile(`Invalid domain[\s\n]+'cdn--'`),
			},
		},
	})
}

func TestResourceNameFunction_NameWithHyphen(t *testing.T) {
	t.Parallel()
	// Test that a name containing a hyphen causes an error
	resource.UnitTest(t, resource.TestCase{
		TerraformVersionChecks: []tfversion.TerraformVersionCheck{
			tfversion.SkipBelow(tfversion.Version1_8_0),
		},
        ...
        `,
				ExpectError: regexp.MustCompile(`Invalid name 'cdn-':[\s\n]+must contain`),
			},
		},
	})
}
