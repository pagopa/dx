...
**Always read the local documentation files** before generating code. These files are the authoritative source for DX best practices in this workspace.

Read these files via `read_file` **before generating any code**:
...
If a file does not exist, note it in the README with `<!-- local-missing: <filename> -->` and continue without it.

**Do NOT use**: DX Search API, `fetch_webpage`, or internal knowledge for best practices — the Module-first rule in step 2 is the only exception (always uses the Registry API or MCP).
