...
- **Internal-only actions/workflows (same repository, never called externally)**: Reference sibling actions and reusable workflows with the self-repository `$/` syntax (e.g. `uses: $/actions/csp-login`, `uses: $/.github/workflows/static_analysis.yaml`), without a `@ref` suffix. `$/` always resolves to the exact commit currently running, so it never needs to track `main` and automatically stays in sync even when the caller is pinned to a tag or full commit SHA. Requires Actions runner >= 2.336.0.
  - **Important caveat**: `$/` always resolves against the **top-level caller's repository and commit**, even through nested composite actions and reusable workflows. `pagopa/dx` is a shared workflow/action provider consumed directly by many other repositories (e.g. via `uses: pagopa/dx/.github/workflows/infra_apply.yaml@main`). Do **not** use `$/` inside any workflow that declares `on: workflow_call` or any composite action under `actions/` (or one it calls, transitively) that is part of the publicly-versioned, cross-repo-consumable surface — a `$/` reference there would resolve to the *external caller's* repository and fail. Reserve `$/` for `_`-prefixed orchestrator workflows and other files that are only ever triggered by events in this same repository and can never be a `uses:` target for another repository.
  - For these publicly-consumable workflows and actions, keep using `pagopa/dx/...@main` (acceptable for internal-to-the-org dependencies during development or when part of the same lifecycle as the caller).
