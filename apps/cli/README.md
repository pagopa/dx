...
To inspect the files generated by the Plop generator integration tests, set `DX_TEST_KEEP_ARTIFACTS=1` before running the test command.

```bash
DX_TEST_KEEP_ARTIFACTS=1 pnpm exec vitest run
```

When the flag is enabled, the temporary directory is preserved after `afterAll` and its path is printed to the console.
