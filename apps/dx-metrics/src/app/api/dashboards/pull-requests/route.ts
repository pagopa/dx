// Pull request dashboard API route — returns aggregated PR metrics for a given repo/days window.
...
export async function GET(req: NextRequest) {
  ...
  try {
    const [cards, leadTime, counts, quality] = await Promise.all([
      fetchPrSummary(fullName, days),
      fetchLeadTimeData(fullName, days),
      fetchPrCountData(fullName, days),
      fetchPrQualityData(fullName, days),
    ]);

    return NextResponse.json({ cards, ...leadTime, ...counts, ...quality });
  } catch (error) {
...
async function fetchLeadTimeData(fullName: string, days: number) {
...
async function fetchPrCountData(fullName: string, days: number) {
...
async function fetchPrQualityData(fullName: string, days: number) {
...
async function fetchPrSummary(fullName: string, days: number) {
...
/** Converts PostgreSQL numeric strings to JS numbers for Recharts compatibility. */
