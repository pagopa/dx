...
The diagram below shows a high-level view of the flow explained in this
document.
...
The diagram below shows what `dx init` provisions and how the resulting entities
...
flowchart TD
    ...
    secrets["Environment Secrets<br/>(Azure / AWS credentials)"]
    app["GitHub App"]
    kv["Azure Key Vault<br/>(App ID · Installation ID · Private Key)"]
    runner["Self-hosted GitHub Runner"]
    rg["Azure Resource Group"]

    repo -->|contains| envs
    envs -->|store| secrets
    secrets -->|"authenticate CI/CD workflows to"| rg
    repo -->|uses| runner
    app -->|credentials stored in| kv
    kv -->|authenticates| runner
    runner -->|hosted in| rg
```
...
This step requires access to the GitHub App settings, which is available only to
App Administrators. Ask your Engineering Leader to retrieve the values and share
them with you before proceeding.
...
In PagoPA context, you must be able to connect VNets with DDos protection plans.

To do so, be sure to join the
[group](https://github.com/search?q=repo%3Apagopa%2Feng-azure-authorization%20sec-p-adgroup-externals&type=code)
that gives you the required roles.
...
This step involves your Engineering Leader only.
