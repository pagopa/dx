...
means any caller can invoke your Function App. The DX team has shipped an update
...
The module configures `auth_settings_v2` under the hood, wiring the Entra ID
v2.0 endpoint and an allowlist of caller client IDs. Only APIM identities
explicitly listed in `allowed_callers_client_ids` can authenticate.
...
| Blast radius   | All APIs share the same key | Per-caller allowlist              |
...
👉
[**Authenticating APIM to Function Apps with Managed Identity**](/docs/infrastructure/azure/integrating-services/apim-function-app-authentication)
