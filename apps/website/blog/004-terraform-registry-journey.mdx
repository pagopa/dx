...
This led us to explore publishing our modules and provider to the Terraform Registry, all while preserving our monorepo development workflow.

In this post, we'll share our journey, challenges, and the technical solutions we implemented to bridge the gap between monorepo development and the Terraform Registry requirements.
...
1. **Code Duplication**: Teams were copying and pasting Terraform code across projects, leading to inconsistencies and maintenance overhead.
...
The core challenge was to reconcile our monorepo development workflow with the Terraform Registry's requirement for individual repositories.
...
The heart of our solution is an automated [GitHub Action workflow](https://github.com/pagopa/dx/blob/main/.github/workflows/_release-bash-modules-to-subrepo.yaml) that handles synchronization between our monorepo and individual repositories. When changes are made to a module in our monorepo, the workflow detects these changes and pushes them to a dedicated sub-repository following the Terraform Registry naming convention.
...
We're investing in improved testing and validation workflows to ensure the highest quality modules possible. And finally, we're developing more complete usage examples to help teams get started quickly with the right patterns.
...
Have questions about our approach or want to see our modules? Check out our [Terraform Registry namespace](https://registry.terraform.io/namespaces/pagopa-dx) or join the conversation in our `#team_devex_help` channel.
