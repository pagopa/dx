...
has been released by the DevEx team to enable customer in easy and
straightforward Service Bus setup!

This module abstracts inner complexity such as networking, authentication and
scaling. In fact, by choosing `l` tier (which matches `Premium` SKU) the module
...
The module also support `m` tier (`Standard` SKU), but it is reccomended to use
this tier only in test environments as the resource cannot be isolated in terms
of networking, and remains on public internet.
...
you should already be familiar with. By passing names of queues, topics and
subscriptions, the IAM complexity is taken away.
...
has been updated to add needed roles to apply Terraform code to a centralized
Service Bus instance, if you have one.
