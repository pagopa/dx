...
The action checks for environment variables to determine which login to perform:
...
The token is **not** exported as a job-wide `GITHUB_TOKEN` anymore. Read it from
the `github_app_token` output and pass it explicitly only to the steps that
