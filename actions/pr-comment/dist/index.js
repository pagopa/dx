 ...
 */
async function createComment(octokit, context, body) {
    try {
 ...
 */
async function deleteMatchingComments(octokit, context, searchPattern) {
    try {
        ...
        const matchingComments = comments.filter((comment) => (comment.body ?? "").toLowerCase().includes(normalizedPattern));
        ...
        warning(`Failed to delete existing comments: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 ...
 */
function resolveCommentBody(inputs) {
    ...
    if (inputs.commentBodyFile) {
        try {
            ...
            throw new Error(`Failed to read comment body file: ${inputs.commentBodyFile}. Error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
async function run() {
    try {
        ...
        info(`Running on PR #${context.issueNumber} in ${context.owner}/${context.repo}`);
        // Get GitHub token and create client
        const token = getInput("github-token") || process.env.GITHUB_TOKEN;
        if (!token) {
            throw new Error("GitHub token not found. Please provide github-token input or GITHUB_TOKEN environment variable");
        }
...
function validateFilePath(filePath) {
    ...
    for (const pattern of sensitivePatterns) {
        if (normalizedPath.includes(pattern)) {
            throw new Error(`File path "${filePath}" contains potentially sensitive pattern "${pattern}".`);
        }
    }
}
// Run the action
