...
from harbor_mod.convert.discover import DiscoverError
from harbor_mod.convert.run import (
    ConvertOptions,
    TaskNameCollision,
    apply_run,
...
def test_check_host_environment_apple_requires_arm64(monkeypatch):
    monkeypatch.setattr("harbor_mod.convert.run.platform.machine", lambda: "x86_64")
...
def test_check_host_environment_apple_requires_container_cli(monkeypatch):
    monkeypatch.setattr("harbor_mod.convert.run.platform.machine", lambda: "arm64")
    monkeypatch.setattr("harbor_mod.convert.run.shutil.which", lambda name: None)
    err = check_host_environment("apple-container")
...
def test_check_host_environment_apple_ready(monkeypatch):
    monkeypatch.setattr("harbor_mod.convert.run.platform.machine", lambda: "arm64")
    monkeypatch.setattr(
        "harbor_mod.convert.run.shutil.which", lambda name: "/usr/bin/container"
    )
