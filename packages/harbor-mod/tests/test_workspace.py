...
from harbor_mod.convert.workspace import WorkspaceError, compose_workspace


def test_composes_base_overlay_and_files(tmp_path):
    env = tmp_path / "env"
    base = tmp_path / "base"
    ...
    overlay.mkdir()
    (overlay / "c.txt").write_text("ov-c")
    f = tmp_path / "file.txt"
    f.write_text("file")

    created = compose_workspace(env, workspace_dir=base, overlays=[overlay], files=[f])
    assert sorted(created) == ["a.txt", "c.txt", "file.txt", "sub/b.txt"]
    assert (env / "a.txt").read_text() == "base-a"
    assert (env / "c.txt").read_text() == "ov-c"
...
def test_collision_between_layers_rejected(tmp_path):
    ...
    overlay = tmp_path / "overlay"
    overlay.mkdir()
    (overlay / "a.txt").write_text("ov")

    with pytest.raises(WorkspaceError, match="collision"):
        compose_workspace(env, workspace_dir=base, overlays=[overlay])
...
def test_same_filename_in_different_dirs_not_collision(tmp_path):
    ...
    created = compose_workspace(env, overlays=[d1, d2])
