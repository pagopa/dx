...
* the **writer** (``harbor_mod.convert``) generates a Task directory whose
...
* the **reader** (``harbor_mod.jobs``) reads those collected artifacts back
...
#: is the exact list Harbor must transfer for ``harbor_mod.jobs`` to read a
